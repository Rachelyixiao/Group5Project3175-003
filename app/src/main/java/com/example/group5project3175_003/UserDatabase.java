...
public class UserDatabase extends SQLiteOpenHelper {
    ...
    final static String T1COL_10 = "Balance";
    final static String T1COL_11 = "DailyAllow";
    ...
    public boolean add(String fn, String ln, String email, String psw, String curr, String ala,
                       String acname, double Income, double Saving, double Balance,double dailyIni) {
        ...
        values.put(T1COL_10, Balance);
        values.put(T1COL_11,dailyIni);
...
/*
    public Cursor viewTodayEx(int id){
        SQLiteDatabase sqLiteDatabase = this.getReadableDatabase();


        //Cursor c = sqLiteDatabase.query(TABLE4_NAME,new String[]{"DEid","DEDate","DE_amount","DE_UID"},"DE_UID=?"+id,null, null,null,null);
        Cursor c = sqLiteDatabase.rawQuery("SELECT * FROM "+TABLE4_NAME+" WHERE DE_UID = "+ id,null);
        return  c;
    }
*/
 ...
 */
    ...
    public boolean updateUserData(String loginID, double balance) {
        SQLiteDatabase sqLiteDatabase = this.getWritableDatabase();
        ContentValues values = new ContentValues();
        values.put(T1COL_10, balance);
    ...
    public Cursor viewDE(String id){
        SQLiteDatabase sqLiteDatabase = this.getReadableDatabase();
        Cursor c = sqLiteDatabase.rawQuery("SELECT * FROM "+TABLE4_NAME +" WHERE DE_UID = "+ id,null);
        return c;
    }
    public Cursor viewUserData(String id){
        SQLiteDatabase sqLiteDatabase = this.getReadableDatabase();
        Cursor c = sqLiteDatabase.rawQuery("SELECT * FROM "+TABLE1_NAME +" WHERE UID = "+ id,null);
        return c;
    }
    public boolean updateDailyAllow(int id,Double DailyAllow) {
        SQLiteDatabase sqLiteDatabase = this.getWritableDatabase();
        ContentValues values = new ContentValues();
        values.put(T1COL_11, DailyAllow);
        int d = sqLiteDatabase.update(TABLE1_NAME, values, "UID=?", new String[]{Integer.toString(id)});
        return (d > 0);
    }
